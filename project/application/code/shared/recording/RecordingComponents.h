...
// 録画モジュールの「データ」層。
//
// このヘッダが shared に置かれているのは、SessionInfo / TrackInfo が
// session.json の受け渡し形式そのものであり、録画アプリが書き
// 再生アプリが読むためである（SessionManifest.h が両方向の I/O を提供する）。
// RecordingSettings は録画アプリの UI 設定だが、ECS 移行時に同じ Component 群へ
// 昇格させる想定なので、データ層として 1 ファイルにまとめたままにしている。
//
